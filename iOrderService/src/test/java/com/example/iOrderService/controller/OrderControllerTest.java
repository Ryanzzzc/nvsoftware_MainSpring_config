...
public class OrderControllerTest {
    ...
    @BeforeEach
    void setupTest() throws IOException {  //put common method for every test
        reduceQuantity();
        getProductResponse();
        doPayment();
        getPaymentResponse();
        ...
        wireMockExtension.stubFor( WireMock.get("/payments/1")
                .willReturn(WireMock.aResponse()
                        ...
                        .withBody(StreamUtils.copyToString(OrderControllerTest.class.getClassLoader()
                                .getResourceAsStream("MockData/paymentResponse.json"), Charset.defaultCharset())
                        )
                )
        );
    }
    ...
    private void getProductResponse() throws IOException {
        wireMockExtension.stubFor( WireMock.get("/product/1")
                .willReturn(WireMock.aResponse()
                        ...
                        .withHeader("Content Type", MediaType.APPLICATION_JSON_VALUE)
                        .withBody(StreamUtils.copyToString(OrderControllerTest.class.getClassLoader()
                                .getResourceAsStream("MockData/productResponse.json"), Charset.defaultCharset())
                                )
                        )
        );
    ...
    private void reduceQuantity() {
        wireMockExtension.stubFor(WireMock.put("/products/reduceQuantity")
                .willReturn(WireMock.aResponse()
                        .withStatus(HttpStatus.OK.value())
                        .withHeader("Content Type", MediaType.APPLICATION_JSON_VALUE)
                )
